...
class DpathIo(implicit val conf: SodorConfiguration) extends Bundle() 
{
   ...
   //cyp_modify_begin
   val tpu_ctl = Flipped(new MTodpathio())
   //cyp_modify_end
}
...
class DatPath(implicit conf: SodorConfiguration) extends Module
{
   ...
   when (!(io.ctl.stall||io.tpu_ctl.stall))
   {
   ...
   //cyp_modify_begin
   val tpu_fun = Wire(UInt(2.W))
   
   tpu_fun := MuxCase(0.U, Array(
                  (io.ctl.alu_fun === TPU_PRE)  -> 1.U,
                  (io.ctl.alu_fun === TPU_ACT)  -> 2.U
   ))

   io.tpu_ctl.csignal := tpu_fun
   io.tpu_ctl.addr1 := rs1_data
   io.tpu_ctl.addr2 := rs2_data
   //cyp_modify_end
   ...
   alu_out := MuxCase(0.U, Array(
                  //cyp_modify_begin
                  (io.ctl.alu_fun === ALU_MUL)  -> (alu_op1 * alu_op2).asUInt,
                  (io.ctl.alu_fun === ALU_DIVU) -> (alu_op1 / alu_op2).asUInt,
                  (io.ctl.alu_fun === ALU_REMU) -> (alu_op1 % alu_op2).asUInt,
                  //cyp_modify_end
